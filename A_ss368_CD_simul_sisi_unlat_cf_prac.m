...
% 2019-03-27 sisi wang
clear;clc
%% change detection with NO distractors
% set parameters
clear;clc
PARAMETER.deg = [0.75  0.65  1.5  2.7  3.9];
% PARAMETER.deg = [0.75  0.65  3.3  4.5  5.7];
pix_per_cm = 832/30;
% pix_per_cm = 1920/43;
...
TrialNum = input('please input Trial Number(24):     ');

%% Record EEG
eeg_record = input('recording EEG? ');
if eeg_record == 1 
   daq = setupDaq(); % set up EEG code
end

% EEG_code
fixation_code = 10; % fixation start
sam_ss_code = [23,26,28]; % setsize 3,6,8, respectively--sample onset
test_ss_code = [33,36,38]; % setsize 3,6,8, respectively--test onset
resp_code = [41,42]; % 31-incorrect; 32-correct
...
break_after = 48;
%% quit program
quit_key = 'esc'; %% press this button at any time to quit the experiment
...
for trial = 1:TrialNum
    ...
    elseif mod(trial,break_after) == 1 && trial > 1
        ...
        Screen('TextSize',WINDOW,20);
        Screen('FillRect',WINDOW,[192 192 192]);
        DrawFormattedText(WINDOW,['Take a short break.\n\n' num2str(TrialNum-trial+1) ' trials left to go in this block.\n\nYour response accuracy until now is' num2str(sum(ACC(1:trial-1))/length(ACC(1:trial-1))) '\n\n Button F =  "change", Button J = "no change"\n\nPress SPACE to resume the experient.'],'center','center',[0,0,0]);
        Screen('Flip',WINDOW);
        
        % save all results to a temporal file
        save (fullfile (['Temp_Prac_UnlaterCD368_sub' num2str(SubNum)  '_Block' num2str(BlockNum) '_miniblock' num2str((trial-1)/break_after) 'cf.mat']));
    ...
    % % EEG_code
    if eeg_record == 1
        err = DaqDOut(daq.deviceNum, daq.eventPort, fixation_code);
    end
...
%     deg_l(:,3) = [240 260 280 300];
    ...
    % % EEG_code--sample onset
    if eeg_record == 1
        err = DaqDOut(daq.deviceNum, daq.eventPort, sam_ss_code(PARAMETER.setsize(trial)));
    end
    ...
    % % EEG_code--test onset
    if eeg_record == 1
        err = DaqDOut(daq.deviceNum, daq.eventPort, test_ss_code(PARAMETER.setsize(trial)));
    end
    ...
    if ACC(trial) == 1
        DrawFormattedText(WINDOW,('Correct!'),'center','center',[0 0 255],[],[],[],[]);
    elseif ACC(trial) == 0
    ...
    % % EEG_code--response
    if eeg_record == 1
        err = DaqDOut(daq.deviceNum, daq.eventPort, resp_code(ACC(trial)+1));
    end
...
save (fullfile (['Prac_UnlaterCD368_sub' SubNum  '_' num2str(BlockNum) '_cf.mat']));
Screen('CloseAll');
